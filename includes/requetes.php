<?php
    ...
    $allFriends = "SELECT u.num_user, u.pseudo, u.photo, f.statut FROM sae_friendship AS f JOIN sae_users AS u ON u.num_user = f.user_id OR u.num_user = f.friend_id WHERE (f.user_id = :num_user OR f.friend_id = :num_user) AND (f.user_id != :num_user AND f.friend_id = :num_user) AND u.num_user != :num_user;";
    ...
    $listUsersPseudo = "SELECT num_user, pseudo, photo FROM sae_users WHERE num_user NOT IN (SELECT friend_id FROM sae_friendship WHERE user_id = :idUser) AND num_user <> :num_user ORDER BY pseudo;";
