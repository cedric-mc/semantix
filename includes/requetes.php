<?php
    ...
    $scoreProfil = "SELECT MIN(score) AS minS, MAX(score) AS maxS, AVG(score) AS avgS, COUNT(score) AS nbParties FROM sae_scores s, sae_users u WHERE u.num_user = s.num_user AND pseudo = :pseudo;";
    ...
    $allFriends = "SELECT DISTINCT u.num_user, pseudo, u.email, u.annee_naissance, u.photo, f.statut, f.user_id AS creatorF, f.friend_id AS acceptF FROM sae_users u JOIN sae_friendship f ON u.num_user = f.friend_id OR u.num_user = f.user_id WHERE (f.user_id = :num_user OR f.friend_id = :num_user) AND u.num_user <> :num_user;";
    ...
    $friendSearch = "SELECT u.num_user, pseudo, u.photo, CASE WHEN f.statut = 0 AND f.user_id = :num_user THEN 0 WHEN f.statut = 1 THEN 1 WHEN f.statut IS NULL THEN 2 END AS statut FROM sae_users AS u LEFT JOIN sae_friendship AS f ON (u.num_user = f.user_id OR u.num_user = f.friend_id) AND (f.user_id = :num_user OR f.friend_id = :num_user) WHERE u.num_user <> :num_user AND pseudo LIKE CONCAT('%', :search_string, '%') ORDER BY pseudo;";
